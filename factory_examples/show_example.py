"""
Модуль случайного выбора ОДНОГО задания из перечня методов модуля examples.py
для показа в окне приложения Порешайка.
"""
...
class Show_test:
    """Класс генерации ОДНОГО задания из перечня методов модуля examples.py."""
    ...
    def get_example(self, f_1=0, f_2=0, f_3=0, f_4=0, f_5=0, f_6=0):
        """
        Основной метод генерации ОДНОГО задания из перечня методов модуля examples.py.
        На вход получает логические значения 0 или 1 (по положению кнопок-флажков оконного приложения).
        Возвращает задание сгенерированное методом, который случайным образом выбран из списка методов модуля examples,
        """
        ...
        # двумерный список методов модуля examples, соответствующих кнопкам-флажкам приложения
        # (6 кнопок-флажков -> 6 списков методов)
        lst_func = [
            [
        ...
        # результирующий список методов модуля examples
